.. zephyr:code-sample:: led-strip-single
   :name: Single RGB LED Smooth Glow
   :relevant-api: led_strip_interface

   Smooth, gentle glowing animation for a single RGB LED (3 separate R, G, B LEDs).

Overview
********

This sample application demonstrates a smooth, gentle glowing animation for a single RGB LED 
composed of three separate LEDs (one red, one green, one blue). The LED smoothly transitions 
between colors at 25% brightness for a calm, ambient effect:

- **Red** → **Blue** (500ms smooth fade)
- **Blue** → **Green** (500ms smooth fade)
- **Green** → **Red** (500ms smooth fade)

The cycle continuously repeats, creating a slow, soothing glow with no harsh flashing.
Total cycle time: 1.5 seconds per complete red→blue→green→red loop.
