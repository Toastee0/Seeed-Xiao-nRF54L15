...
#define DMIC_BLOCK_SIZE_MS 100
#define DMIC_BLOCK_SIZE_SAMPLES (DMIC_SAMPLE_RATE * DMIC_BLOCK_SIZE_MS / 1000)
#define DMIC_BLOCK_SIZE_BYTES (DMIC_BLOCK_SIZE_SAMPLES * DMIC_BYTES_PER_SAMPLE)
#define DMIC_BLOCK_COUNT 8
...
static uint8_t *latest_audio_buffer = NULL;
...
static void dmic_ble_audio_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		/* Update the shared buffer for BLE streaming */
		k_mutex_lock(&audio_data_mutex, K_FOREVER);
		
		/* Don't free the old buffer - let mem slab manage it */
		latest_audio_buffer = buffer;
		latest_audio_size = size;
		dmic_data_ready = true;
		
		k_mutex_unlock(&audio_data_mutex);

		/* Monitor audio level every 50 blocks (~5 seconds at 100ms blocks) */
		block_count++;
		if (block_count >= 50) {
		...
		/* Note: buffer is managed by the mem slab and will be reused automatically */
	}
}
...
static void send_data(struct broadcast_source_stream *source_stream)
{
	...
	if (dmic_data_ready && latest_audio_buffer != NULL) {
		...
		/* Note: Don't free buffer here - DMIC thread manages it */
	} else {
		/* Fallback to silence if no DMIC data available */
		const unsigned int num_samples = (frame_duration_us * freq_hz) / USEC_PER_SEC;
